...
#if defined(NDEBUG)
# error "Bitriad cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x67da48f7a10c0cfe1b5eb3ab0c520ae9ba140ea94452ded38e513528278438f8");
...
const string strMessageMagic = "Bitriad Signed Message:\n";
...
{
    // Bitriad: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Bitriad: 420k blocks in ~2 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Bitriad: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Bitriad: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bitriad: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bitriad: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x566645f7e3484a71226a526e04286f8f09b364f651e156f0c00e53e90f59ad19");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "When i created bitriad coin for you";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1572470201;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2090596540;

        if (fTestNet)
        {
            block.nTime    = 1572470069;
            block.nNonce   = 393143185;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
{
    ...
    printf("block.GetHash = %s\n", block.GetHash().ToString().c_str());
        ...
        assert(block.hashMerkleRoot == uint256("0xb80cf9740b85c4cab4ac2918abbb03bdffd783717ee73b07df6d1b6a3c18e0bf"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Bitriad: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitriadMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitriadMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BitriadMiner(CWallet *pwallet)
{
    printf("BitriadMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bitriad-miner");
    ...
    try { loop {
        ...
        printf("Running BitriadMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BitriadMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitriadMiner, pwallet));
}
